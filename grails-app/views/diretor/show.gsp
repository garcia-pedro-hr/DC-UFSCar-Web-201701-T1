	...
	<body>
		...
		<div id="show-diretor" class="content scaffold-show" role="main">
			...
			<ol class="property-list diretor">
				...
				<g:if test="${diretorInstance?.filmes}">
				<li class="fieldcontain">
					<span id="filmes-label" class="property-label"><g:message code="diretor.filmes.label" default="Filmes" /></span>
					
						<g:each in="${diretorInstance.filmes}" var="f">
						<span class="property-value" aria-labelledby="filmes-label"><g:link controller="DVD" action="show" id="${f.id}">${f?.encodeAsHTML()}</g:link></span>
						</g:each>
